...
{
    //
    public function downloadDocument(Request &$request, $attestation_id) {
        $session = $request->session()->get('triethic');
        $result = DB::table('attestations')
                    ->whereIn('attestations.pointcollecte_id', $session['pointcollectes'])
                    ->where('attestations.id', '=', $attestation_id)
                    ->get();
    ...
    public function list(Request &$request) {
        ...
        $result = DB::table('attestations')
                    ->whereIn('attestations.pointcollecte_id', $pointcollecte_ids)
                    ->get();

        return response()->json(['status' => false, 'message' => '', 'result' => $result], 200);
    }
    public function downloadDocuments(Request &$request) {
        $allowedFields = ['attestation_ids'   => 'string'];
        $request->validate($allowedFields);
        $session = $request->session()->get('triethic');
        $result = DB::table('attestations')
                    ->whereIn('attestations.pointcollecte_id', $session['pointcollectes'])
                    ->whereIn('attestations.id', json_decode($request->get('attestation_ids')))
        ...
        foreach($result AS &$value) {
            array_push($files, ['file' => Paths::clientDocuments($value->document)
                              , 'name' => $value->annee.'-attestation.pdf'
                            ]);
        }
